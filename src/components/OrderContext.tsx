...
export default function OrderContext({order, removeItem}: OrderContextProps) {
  return (
    <div>
            ...
            {order.map(item => (
                    <div key={item.id} className="flex justify-between border-t items-center border-gray-200 py-5 last-of-type:border-b ">
                        ...
                        <div>
                                ...
                                <p className="text-lg">
                                    {item.name}-{ formatCurrency(item.price)}</p>
                                    <p className="font-black">
                                            Cantid: {item.quantity} - {formatCurrency(item.price * item.quantity)}
                                        </p>
